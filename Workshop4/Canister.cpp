...
namespace sdds {
    ...
    Canister& Canister::pour(Canister& provide)
    {
        setContent(provide.m_contentName);//set the content of 'this' canister to the provided canister

        if (provide.volume() <= (this->capacity() - this->volume()))
        {
            pour(provide.m_contentVolume);//pour everything out from the canister
            provide.m_contentVolume = 0.0;
        }
        else
        { 
            //reduce the volume of the canister by the remaining capacity of 'this' canister
            provide.m_contentVolume -= (this->capacity() - this->volume()); 
